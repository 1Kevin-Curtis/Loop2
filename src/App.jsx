...
const screens = ["welcome", "profile", "home", "rounds", "ready", "setup", "round", "hole", "review", "plan", "prompt", "practice", "progress"];
...
function Nav({ go }) { return <div className="nav">{[["home", "⛳", "Home"], ["rounds", "◔", "Rounds"], ["setup", "＋", "Play"], ["plan", "◎", "Plan"], ["progress", "◉", "Progress"]].map(([s, i, t]) => <button key={s} onClick={() => go(s)}><span>{i}</span>{t}</button>)}</div>; }
...
function Drill({ title, body }) { return <Card><div className="row"><button className="tickBtn">✓</button><div><h3>{title}</h3><p>{body}</p></div></div></Card>; }
...
function Rounds({ go, back }) {
  return <Phone>
    <Header title="Rounds" sub="Your golf story over time." back={back} />
    <div className="stack bottomGap">
      {sampleRounds.map((round, i) => (
        <Card key={i}>
          <div className="between">
            <div>
              <p className="eyebrow">{round.course}</p>
              <h2>{round.score} ({round.relative})</h2>
            </div>
            <span className="badge">{round.date}</span>
          </div>

          <div className="pills" style={{marginTop:"14px"}}>
            {round.insights.map((x, idx) => (
              <div key={idx} className="pill on" style={{background:"#E6F4EE", color:"#0F2D2E"}}>
                {x}
              </div>
            ))}
          </div>
          ...
          <p className="muted" style={{marginTop:"14px"}}>
            Pattern confidence increases as more rounds are logged.
          </p>
        </Card>
      ))}
    </div>
...
export default function App() {
  ...
  return <div><style>{css}</style>{screen === "welcome" && <Welcome {...props} />} {screen === "profile" && <Profile {...props} />} {screen === "home" && <Home {...props} />} {screen === "rounds" && <Rounds {...props} />} {screen === "ready" && <Ready {...props} />} {screen === "setup" && <Setup {...props} />} {screen === "round" && <Round {...props} />} {screen === "hole" && <Hole {...props} />} {screen === "review" && <Review {...props} />} {screen === "plan" && <Plan {...props} />} {screen === "prompt" && <Prompt {...props} />} {screen === "practice" && <Practice {...props} />} {screen === "progress" && <Progress {...props} />}</div>;
}
