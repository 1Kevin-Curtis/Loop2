...
const COURSE_DATA = {
  ...
  "The Oaks Golf Club": {
    White: {
      yards: [388, 547, 171, 405, 358, 189, 428, 367, 521, 412, 177, 389, 495, 143, 402, 371, 195, 438],
      pars: [4, 5, 3, 4, 4, 3, 4, 4, 5, 4, 3, 4, 5, 3, 4, 4, 3, 4],
      si: [7, 11, 17, 3, 13, 15, 1, 9, 5, 2, 18, 8, 12, 16, 4, 14, 10, 6]
    },
    Yellow: {
      yards: [371, 526, 154, 389, 341, 171, 409, 349, 501, 394, 160, 372, 476, 129, 383, 352, 178, 418],
      pars: [4, 5, 3, 4, 4, 3, 4, 4, 5, 4, 3, 4, 5, 3, 4, 4, 3, 4],
      si: [7, 11, 17, 3, 13, 15, 1, 9, 5, 2, 18, 8, 12, 16, 4, 14, 10, 6]
    },
    Red: {
      yards: [342, 488, 132, 350, 310, 146, 372, 316, 462, 359, 137, 338, 441, 111, 348, 319, 150, 381],
      pars: [4, 5, 3, 4, 4, 3, 4, 4, 5, 4, 3, 4, 5, 3, 4, 4, 3, 4],
      si: [7, 11, 17, 3, 13, 15, 1, 9, 5, 2, 18, 8, 12, 16, 4, 14, 10, 6]
    }
  },
  "Kingswood Golf and Country Club": {
    White: {
      yards: [410, 503, 166, 398, 421, 176, 390, 553, 414, 374, 203, 442, 367, 512, 154, 431, 188, 420],
      pars: [4, 5, 3, 4, 4, 3, 4, 5, 4, 4, 3, 4, 4, 5, 3, 4, 3, 4],
      si: [9, 7, 17, 11, 3, 15, 13, 1, 5, 12, 16, 2, 14, 8, 18, 4, 10, 6]
    },
    Yellow: {
      yards: [392, 486, 151, 381, 402, 162, 371, 535, 397, 356, 184, 423, 351, 493, 140, 410, 171, 401],
      pars: [4, 5, 3, 4, 4, 3, 4, 5, 4, 4, 3, 4, 4, 5, 3, 4, 3, 4],
      si: [9, 7, 17, 11, 3, 15, 13, 1, 5, 12, 16, 2, 14, 8, 18, 4, 10, 6]
    },
    Red: {
      yards: [351, 451, 126, 346, 366, 135, 335, 497, 361, 321, 154, 382, 318, 456, 118, 374, 144, 365],
      pars: [4, 5, 3, 4, 4, 3, 4, 5, 4, 4, 3, 4, 4, 5, 3, 4, 3, 4],
      si: [9, 7, 17, 11, 3, 15, 13, 1, 5, 12, 16, 2, 14, 8, 18, 4, 10, 6]
    }
  }
...
,
  "Coulsdon Manor": {
    Yellow: {
      yards:[320,398,148,488,172,344,392,352,508,382,166,401,472,138,412,361,181,426],
      pars:[4,4,3,5,3,4,4,4,5,4,3,4,5,3,4,4,3,4],
      si:[10,4,16,2,14,8,6,12,1,11,17,5,3,15,7,9,13,18]
    }
  },
  "Parkwood Golf Club": {
    Yellow: {
      yards:[309,388,143,476,168,332,384,344,501,377,160,392,463,134,407,355,172,421],
      pars:[4,4,3,5,3,4,4,4,5,4,3,4,5,3,4,4,3,4],
      si:[9,5,17,1,13,7,3,11,15,10,18,4,2,16,6,8,14,12]
    }
  }
...
function getCourseSummary(course, tee) {
  const data = COURSE_DATA[course][tee];
  const totalYards = data.yards.reduce((a,b)=>a+b,0);
  const totalPar = data.pars.reduce((a,b)=>a+b,0);
  return { totalYards, totalPar };
}
...
function Profile({ go, back }) { const [h, setH] = useState(14), [goal, setGoal] = useState("Break 80"); return <Phone><Header title="Your golfer profile" sub="A quick setup so advice feels relevant." back={back} /><div className="stack"><Card><p className="eyebrow">Current handicap</p><div className="counter"><button onClick={() => setH(Math.max(1, h - 1))}>−</button><strong>{h}</strong><button onClick={() => setH(h + 1)}>+</button></div></Card><Card soft><p className="eyebrow">Main goal</p><h2 style={{marginBottom:"12px"}}>{goal}</h2><p style={{marginBottom:"16px"}}>Loop adapts your reviews, plans and prompts around the scoring target you are working towards.</p><div className="pills">{["Break 90", "Break 85", "Break 80", "Single figures"].map(x => <Pill key={x} on={goal === x} click={() => setGoal(x)}>{x}</Pill>)}</div></Card><Btn onClick={() => go("home")}>Continue</Btn></div></Phone>; }
...
function Ready({ go, back }) { const moves = ["Shoulder turns", "Hip openers", "Hamstring sweep", "Wrist circles", "Three easy swings"]; const [completedWarmups, setCompletedWarmups] = useState([]); const toggleWarmup=(m)=>setCompletedWarmups(prev=>prev.includes(m)?prev.filter(x=>x!==m):[...prev,m]); return <Phone><Header title="Ready to Play" sub="Four minutes. Calm body, clear first tee." back={back} /><div className="stack bottomGap"><Card soft><strong>Insight engine connected</strong><p className="muted">Analysing scoring patterns, misses and recovery trends from your saved holes.</p></Card><Card dark><p className="eyebrow light">First tee reset</p><h2>Loosen up and find rhythm</h2><p>Simple movements you can do by the car park or practice green.</p><p className="note">Better starts usually lead to steadier opening holes.</p></Card>{moves.map((m, i) => <Card key={m} soft={completedWarmups.includes(m)}><div className="row"><button className={`tickBtn ${completedWarmups.includes(m) ? "done" : ""}`} onClick={() => toggleWarmup(m)}>✓</button><div><h3>{m}</h3><p className="muted">30–45 seconds. Smooth and easy.</p></div></div></Card>)}<Card><p className="eyebrow">First tee focus</p><h3>Pick a safe target. Commit to one smooth swing.</h3></Card><Btn onClick={() => go("setup")}>Start round</Btn></div><Nav go={go} /></Phone>; }

function Setup({ go, back, course, setCourse, tee, setTee }) {
  const courseNames = Object.keys(COURSE_DATA);
  return <Phone><Header title="Course setup" sub="Choose your course and tee before you start." back={back} /><div className="stack bottomGap"><Card soft><strong>Insight engine connected</strong><p className="muted">Analysing patterns from saved holes and previous rounds.</p></Card><Card dark><p className="eyebrow light">Today’s round</p><h2>Where are you playing?</h2><p>Loop uses the course, tee, par, yardage and stroke index to make your round review more useful without adding GPS or shot tracking.</p></Card><Card><p className="eyebrow">Course</p><select className="courseSelect" value={course} onChange={(e)=>setCourse(e.target.value)}>{courseNames.map((x)=><option key={x} value={x}>{x}</option>)}</select><div className="courseMeta"><strong>{course}</strong><p>Par information, yardages and stroke indexes loaded from course database.</p></div></Card><Card><p className="eyebrow">Tee colour</p><div className="pills">{["White", "Yellow", "Red"].map(x => <Pill key={x} on={tee === x} click={() => setTee(x)}>{x}</Pill>)}</div></Card><Card soft>Hole pars, yardages and stroke indexes are attached to the round, so post-round entry feels less disconnected.</Card><Btn onClick={() => go("round")}>Continue to round capture</Btn></div><Nav go={go} /></Phone>;
}

function HoleGrid({ round, select }) { return <div className="holes">{round.map((h, i) => <button key={h.hole} onClick={() => select(i)} className={`${h.saved ? "saved" : ""}`}><b>{h.hole}</b><span>{h.saved ? "Saved" : "Ready"}</span></button>)}</div>; }
...
function Drill({ title, body, done, toggle }) { return <Card soft={done}><div className="row"><button className={`tickBtn ${done ? "done" : ""}`} onClick={toggle}>✓</button><div><h3>{title}</h3><p>{body}</p></div></div></Card>; }
function Practice({ go, back, round }) { const insight = analyse(round).improvements[0]; const [completedDrills, setCompletedDrills] = useState([]); const toggleDrill=(d)=>setCompletedDrills(prev=>prev.includes(d)?prev.filter(x=>x!==d):[...prev,d]); return <Phone><Header title="Practice plan" sub="Short, practical and built for your next round." back={back} /><div className="stack bottomGap"><Card dark><p className="eyebrow light">Before your next round</p><h2>{insight.title}</h2><p>{insight.action}</p></Card><Drill title="Driver confidence · 10 mins" done={completedDrills.includes("driver")} toggle={() => toggleDrill("driver")} body="Hit 5 tee shots at 70% tempo. Focus only on balance and finishing the swing." /><Drill title="Distance control · 15 mins" done={completedDrills.includes("distance")} toggle={() => toggleDrill("distance")} body="Land 10 wedges inside 15 paces without changing target. The goal is pin-high, not perfect." /><Drill title="Short putts · 10 mins" done={completedDrills.includes("putting")} toggle={() => toggleDrill("putting")} body="Make 20 three-foot putts before leaving the green. Keep the routine the same every time." /><Btn onClick={() => go("progress")}>Mark practice complete</Btn></div><Nav go={go} /></Phone>; }
function Progress({ go, back }) { return <Phone><Header title="Progress" sub="Simple trends. No stats overload." back={back} /><div className="stack bottomGap"><Card dark><p className="eyebrow light">Current pattern</p><h2>Reliable strengths are building</h2><p>Your driving and putting are giving you a stronger base while approach distance control remains the focus.</p></Card><Card><h3>Trend confidence</h3>{[["Round 1", "No three putts", "Strong"], ["Round 2", "Waiting for another completed round", "Building"], ["Round 3", "More rounds unlock confidence trends", "No data"]].map(([r, l, s]) => <div className="trend" key={r}><div><b>{r}</b><p>{l}</p></div><span>{s}</span></div>)}</Card><Btn onClick={() => go("plan")}>View updated plan</Btn></div><Nav go={go} /></Phone>; }
...
export default function App() {
  ...
  const setCourse = (nextCourse) => {
    ...
    setRound(buildRound(nextCourse, tee));
    setHole(0);
    setLastSaved(null);
  };
...
.header p{font-size:14px;color:rgba(15,45,46,.58);margin:2px 0 0}
...
.muted{color:rgba(15,45,46,.58);font-size:15px;line-height:1.45;margin-top:7px}
.eyebrow{font-size:14px;text-transform:uppercase;letter-spacing:.08em;font-weight:800;color:rgba(15,45,46,.55);margin-bottom:8px}
...
.nav button{border:0;background:transparent;color:rgba(15,45,46,.72);font-size:15px;display:flex;flex-direction:column;gap:3px;align-items:center}
.nav span{font-size:17px}
.pills{display:flex;flex-wrap:wrap;gap:8px;margin-top:10px}
.coursePills .pill{max-width:100%;text-align:left;line-height:1.25}.courseSelect{width:100%;height:54px;border-radius:16px;border:1px solid rgba(15,45,46,.12);padding:0 14px;font-size:15px;background:white;margin-top:10px}.courseMeta{margin-top:14px}
.pill{border:1px solid rgba(126,219,180,.25);border-radius:999px;background:var(--off);color:rgba(15,45,46,.78);padding:10px 12px;font-size:15px}
...
.journey span{display:block;font-size:15px;margin-top:8px;color:rgba(15,45,46,.72);font-weight:700}.journey em{height:1px;background:rgba(126,219,180,.5);flex:.5;margin-top:19px}
.row{display:flex;gap:14px;align-items:flex-start}.num,.tick{display:grid;place-items:center;min-width:36px;height:36px;border-radius:999px;background:var(--deep);color:white}.tick{background:var(--mint)}.tickBtn{width:42px;height:42px;border-radius:999px;border:0;background:var(--pale);color:var(--mint);font-weight:800;font-size:18px;transition:all .2s ease}.tickBtn.done{background:var(--mint);color:white;transform:scale(1.05)}
.note{background:rgba(255,255,255,.1);border-radius:16px;padding:12px;margin-top:14px;font-size:14px}.savedMsg{background:var(--pale);color:var(--mint);border-radius:16px;padding:12px 14px;font-size:14px;font-weight:700}
.between{display:flex;align-items:flex-start;justify-content:space-between;gap:12px}.badge,.good{border-radius:999px;background:var(--off);color:rgba(15,45,46,.68);padding:7px 10px;font-size:15px;white-space:nowrap}.good{background:var(--pale);color:var(--mint)}
...
.holes{display:grid;grid-template-columns:repeat(3,1fr);gap:16px}.holes button{border:1px solid rgba(126,219,180,.18);background:var(--off);color:rgba(15,45,46,.58);border-radius:22px;padding:20px 8px;min-height:92px;font-size:16px}.holes button.saved{background:var(--pale);color:var(--mint);border:1px solid var(--soft)}.holes b{display:block;font-size:28px;font-weight:800;line-height:1}.holes span{display:block;font-size:10px;margin-top:3px}
.two{display:grid;grid-template-columns:1fr 1fr;gap:10px}.trend{display:flex;justify-content:space-between;gap:12px;padding:12px;border-radius:16px;background:var(--off);margin-top:10px}.trend p{color:rgba(15,45,46,.58)}.trend span{background:white;border-radius:999px;padding:7px 10px;font-size:15px;color:rgba(15,45,46,.68);height:max-content}
