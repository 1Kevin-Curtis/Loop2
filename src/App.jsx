...
function Profile({go}) {
  ...
  return <div className="screenWrap">
    <Header title="Your golfer profile" sub="Loop adapts around your golf as more rounds are played."/>
...
function Setup({go,course,setCourse,tee,setTee}) {
...
function Round({go}) {
  ...
  return <div className="screenWrap">
    ...
    <div className="holes">
      {holes.map(h => (
        <button key={h.hole} className={`holeCard ${h.hole===1 ? "current" : ""}`} onClick={()=>go("hole")}>
...
function Hole({go}) {
  const [selectedTag,setSelectedTag] = useState("Good drive");
  const par = 4;
  ...
  return <div className="screenWrap">
    ...
    <div className="stack">
      ...
      <Card>
        ...
        <p>Putting confidence continuing across multiple rounds.</p>
      </Card>
    </div>
...
export default function App() {
  ...
  return <>
    ...
    {screen==="setup" && <Setup go={go} course={course} setCourse={setCourse} tee={tee} setTee={setTee}/>}
...
.holeCard.current{
  ...
  color:white;
  border-color:#0F2D2E;
  box-shadow:0 12px 28px rgba(15,45,46,.16);
}
...
.tee.active{
  box-shadow:0 8px 18px rgba(15,45,46,.12);
}
