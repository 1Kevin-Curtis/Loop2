...
function Progress({ go, back }) { return <Phone><Header title="Progress" sub="Simple trends. No stats overload." back={back} /><div className="stack bottomGap"><Card dark><p className="eyebrow light">Current pattern</p><h2>Reliable strengths are building</h2><p>Your driving and putting are giving you a stronger base while approach distance control remains the focus.</p></Card><Card><h3>Trend confidence</h3>{[["Round 1", "No three putts", "Strong"], ["Round 2", "Waiting for more rounds", "Building"], ["Round 3", "Keep playing to unlock trends", "No data"]].map(([r, l, s]) => <div className="trend" key={r}><div><b>{r}</b><p>{l}</p></div><span>{s}</span></div>)}</Card><Btn onClick={() => go("plan")}>View updated plan</Btn></div><Nav go={go} /></Phone>; }
...
.header p{font-size:13px;color:rgba(15,45,46,.58);margin:2px 0 0}
...
.muted{color:rgba(15,45,46,.58);font-size:14px;line-height:1.45;margin-top:7px}
.eyebrow{font-size:13px;text-transform:uppercase;letter-spacing:.08em;font-weight:800;color:rgba(15,45,46,.55);margin-bottom:8px}
...
.pill{border:1px solid rgba(126,219,180,.25);border-radius:999px;background:var(--off);color:rgba(15,45,46,.78);padding:10px 12px;font-size:14px}
...
.note{background:rgba(255,255,255,.1);border-radius:16px;padding:12px;margin-top:14px;font-size:13px}.savedMsg{background:var(--pale);color:var(--mint);border-radius:16px;padding:12px 14px;font-size:14px;font-weight:700}
