...
const screens = ["welcome","profile","home","rounds","roundStory","ready","setup","round","hole","review","plan","practice","progress"];
...
function Profile({go}) {
  ...
  return <div className="screenWrap">
    ...
    <div className="stack"><Card soft><h3>Storyline</h3><p>Your rounds are now connected together so Loop can identify progression patterns over time.</p></Card>
      ...
      <Card soft>
        ...
        <button className="storyLink" onClick={()=>{setSelectedRound(storylineData[i]);go("roundStory")}}>View story</button><div className="pills">
...
function Rounds({go,setSelectedRound}) {
  return <div className="screenWrap">
...
const storylineData = [
  {
    ...
    relative:"+11",
    date:"14 May 2026",
    improved:"Driving accuracy improving over recent rounds.",
    stable:"Putting confidence holding steady.",
    focus:"Approach distance control still costing shots.",
    confidence:"Building pattern",
    nextFocus:"Take one more club into greens."
  },
  {
    id:2,
    course:"Parkwood Golf Club",
    ...
    relative:"+14",
    date:"7 May 2026",
    improved:"Short-game recovery becoming more reliable.",
    stable:"Tee-shot confidence stable.",
    focus:"Penalty shots increasing.",
    confidence:"Early signal",
    nextFocus:"Reduce aggressive tee-line choices."
  }
];



function RoundStory({go,selectedRound}) {

  if(!selectedRound) return null;

  return <div className="screenWrap">
    <Header
      title={selectedRound.course}
      sub={selectedRound.date}
    />

    <div className="stack">

      <Card dark>
        <h2>{selectedRound.score} ({selectedRound.relative})</h2>
        <p>Connected improvement story built from recent rounds.</p>
      </Card>

      <Card>
        <p className="eyebrow">What improved</p>
        <h3>{selectedRound.improved}</h3>
      </Card>

      <Card>
        <p className="eyebrow">Stable strength</p>
        <h3>{selectedRound.stable}</h3>
      </Card>

      <Card soft>
        <p className="eyebrow">Current focus</p>
        <h3>{selectedRound.focus}</h3>
      </Card>

      <Card>
        <p className="eyebrow">Trend confidence</p>
        <h3>{selectedRound.confidence}</h3>
        <p>Confidence strengthens as more rounds are logged and connected together.</p>
      </Card>

      <Card soft>
        <p className="eyebrow">Next round focus</p>
        <h3>{selectedRound.nextFocus}</h3>
      </Card>

      <Btn onClick={()=>go("plan")}>View updated plan</Btn>
...
export default function App() {
  ...
  return <>
    ...
    {screen==="rounds" && <Rounds go={go} setSelectedRound={setSelectedRound}/>} {screen==="roundStory" && <RoundStory go={go} selectedRound={selectedRound}/>}
...
.storyLink{margin-top:14px;border:0;background:none;color:#17A589;font-weight:700;padding:0;font-size:14px}.eyebrow{
